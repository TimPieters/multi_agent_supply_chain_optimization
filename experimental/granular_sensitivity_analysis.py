...
matplotlib.use(backend='Agg')  # Use non-interactive backend for saving plots
...
class GranularSensitivityAnalysis:
    ...
    def sobol_analysis_granular(self, base_data, factor_range=(0.7, 1.3), n_samples=100, calc_second_order=False, parallel=False, n_processors=None):
        ...
        print(f"  Evaluating {len(param_values_factors)} Sobol samples{' in parallel' if parallel else ''}...")
        valid_results_count = 0

        if parallel:
            n_jobs = n_processors or os.cpu_count() or 1
            print(f"  Running parallel evaluation on {n_jobs} cores...")

            def eval_sample(idx, X_factors):
                try:
                    current_data = copy.deepcopy(base_data)
                    for j, name in enumerate(problem['names']):
                        factor = X_factors[j]
                        p_type, p_idx = param_indices[name]
                        temp = self._modify_single_coefficient(current_data, p_type, p_idx, factor)
                        if temp is None:
                            raise ValueError(f"Modification failed for {name}")
                        current_data = temp

                    metrics = self._solve_instance_and_get_metrics(current_data)
                    return (idx, metrics['objective_value'] if metrics and 'objective_value' in metrics else np.nan)
                except Exception as e:
                    print(f"Error in parallel sample eval (sample {idx}): {e}")
                    return (idx, np.nan)

            # Prepare iterable with tqdm
            tasks = list(enumerate(param_values_factors))
            tqdm_tasks = tqdm(tasks,
                               desc="Evaluating Sobol Samples (parallel)",
                               total=len(param_values_factors))
            results = Parallel(n_jobs=n_jobs)(
                delayed(eval_sample)(i, X) for i, X in tqdm_tasks
            )
            # Collect
            for i, val in results:
                y[i] = val
            valid_results_count = int(np.sum(~np.isnan(y)))
...
if __name__ == "__main__":
    ...
    print("\n--- Running OAT ---")
    oat_results = granular_sa.one_at_a_time_analysis_granular(baseline_instance, factor_range=(0.8, 1.2), n_samples=5)
    ...
    morris_results = granular_sa.morris_method_granular(baseline_instance, factor_range=(0.8, 1.2), num_trajectories=10, num_levels=4)
    ...
    sobol_results = granular_sa.sobol_analysis_granular(baseline_instance, factor_range=(0.8, 1.2), n_samples=100000, calc_second_order=True,parallel=True,n_processors=18)
